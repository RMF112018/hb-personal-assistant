...
EVIDENCE_DIR = (
    ...
    / "docs"
    / "evidence"
    / "construction-intelligence-phase-08d-agent-data-quality-evaluation"
)
ARCHITECTURE_NOTE = (
    REPO_ROOT
    / "docs"
    / "architecture"
    / "118-phase-08d-agent-data-quality-evaluation-evidence-packet.md"
)
...
UNSAFE_PATTERNS = [
    (
        ...
        re.compile(r"access[_-]?token[\"'\s:=]+[A-Za-z0-9._~-]{12,}", re.I),
    ),
    (
        "refresh_token_assignment",
        re.compile(r"refresh[_-]?token[\"'\s:=]+[A-Za-z0-9._~-]{12,}", re.I),
    ),
    (
        "client_secret_assignment",
        re.compile(r"client[_-]?secret[\"'\s:=]+[A-Za-z0-9._~-]{8,}", re.I),
    ),
...
def write_md(
    path: Path, title: str, payload: dict[str, Any], sections: list[str] | None = None
) -> None:
    lines = [
        f"# {title}",
        "",
...
def source_families_for_name(name: str) -> list[str]:
    lower = name.lower()
    return [
        family for family, terms in SOURCE_FAMILIES.items() if any(term in lower for term in terms)
    ]
...
def table_inventory(conn: sqlite3.Connection, db_path: Path) -> dict[str, Any]:
    ...
    for table in tables:
        ...
        summary = {
            ...
            "has_json_fields": any(
                "json" in c.lower() or c.endswith("_metadata") for c in column_names
            ),
            ...
            "guard_columns": [
                c for c in column_names if c.startswith("no_") or c.endswith("_persisted")
            ],
            "possible_raw_content_risk_fields": [
                c for c in column_names if RISKY_FIELD_RE.search(c)
            ],
        }
    ...
    return {
        ...
        "high_row_count_tables": sorted(
            [s for s in table_summaries if s["row_count"] >= 100],
...
def column_profile(
    conn: sqlite3.Connection, table: str, column: dict[str, Any], row_count: int
) -> dict[str, Any]:
    ...
    usefulness = {
        ...
        "stale_date_rate_90d": round(stale_date_count / row_count, 6)
        if row_count and stale_date_count is not None
        else None,
        "orphan_reference_risk": bool(
            ("id" in name.lower() or "key" in name.lower()) and null_count > 0
        ),
        ...
        "inconsistent_enum_values_indicator": (
            distinct_count > 30 and REVIEW_RE.search(name) is not None
        ),
...
def field_profiles(conn: sqlite3.Connection, tables: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "raw_content_risk_fields": [
            {"table": f["table"], "column": f["column_name"], "metadata_only": True}
...
def source_lineage(inventory: dict[str, Any]) -> dict[str, Any]:
    ...
    for family in SOURCE_FAMILIES:
        matching = [
            ...
            if family in t["source_families"]
            or any(term in t["table"].lower() for term in SOURCE_FAMILIES[family])
        ]
        by_family[family] = {
            ...
            "ingestion_tables": [
                t["table"]
                for t in matching
                if t["functional_area"] in {"ingestion state", "source registry"}
            ],
            "derived_read_model_tables": [
                t["table"]
                for t in matching
                if t["functional_area"]
                not in {"ingestion state", "source registry", "review queues"}
            ],
            ...
            "project_linked_table_count": sum(
                1 for t in matching if t["has_project_entity_reference_fields"]
            ),
            "source_reference_table_count": sum(
                1 for t in matching if t["has_source_reference_fields"]
            ),
...
def generated_output_evidence(inventory: dict[str, Any]) -> dict[str, Any]:
    generated_tables = [
        ...
        for t in inventory["table_summaries"]
        if any(
            term in t["table"].lower()
            for term in ("daily_brief", "research", "synthesis", "evaluation", "output")
        )
    ]
    return {
...
def financial_evidence(inventory: dict[str, Any], profiles: dict[str, Any]) -> dict[str, Any]:
    financial_tables = [
        t for t in inventory["table_summaries"] if "financial" in t["table"].lower()
    ]
    financial_fields = [
        {
            "table": table["table"],
...
def obsidian_evidence(policy: PathPolicy) -> dict[str, Any]:
    ...
    for path in vault.rglob("*"):
        ...
        if suffix == ".md":
            ...
            if any(
                term in rel_lower
                for term in ("daily_brief", "research", "hb personal assistant", "generated")
            ):
                generated_count += 1
            else:
                manual_count += 1
...
def baseline(
    policy: PathPolicy, db_path: Path, commands_run: list[dict[str, Any]]
) -> dict[str, Any]:
    return {
...
def build_packet(output_dir: Path) -> dict[str, Any]:
    ...
    completeness = {
        ...
        "operational_unused_indicator_tables": [
            t["table"] for t in inventory["table_summaries"] if t["row_count"] == 0
        ],
        "data_quality_conclusions_made": False,
    }
    agent_map = {
