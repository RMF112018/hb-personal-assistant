#!/usr/bin/env bash
# Run the forecast CONTEXT package generator for project tropical.
...
echo "[cfr] START run_tropical_context"
echo "[cfr] subproject_root: ${SUBPROJECT_ROOT}"
echo "[cfr] data_root:       ${DATA_ROOT}"

cd "${SUBPROJECT_ROOT}"
PYTHONPATH=src python3 -m construction_financial_review.cli run-context --project tropical

echo "[cfr] END run_tropical_context"
