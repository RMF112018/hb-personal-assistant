#!/usr/bin/env bash
# Run the CROSSWALK-AWARE forecast analysis v2 generator for project tropical.
...
echo "[cfr] START run_tropical_crosswalk_v2"
echo "[cfr] subproject_root: ${SUBPROJECT_ROOT}"
echo "[cfr] data_root:       ${DATA_ROOT}"

cd "${SUBPROJECT_ROOT}"
PYTHONPATH=src python3 -m construction_financial_review.cli run-crosswalk-v2 --project tropical

echo "[cfr] END run_tropical_crosswalk_v2"
