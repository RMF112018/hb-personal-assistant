            ...
            OrderedDict(
                [
                    ...
                    (
                        ...
                        r.get("total_completed_and_stored_to_date"),
                    ),
...
def load_procore_history(context_pkg: Path) -> dict:
    """Group Procore subcontractor pay-app line items by mapped budget key, sorted by period_end.

    Returns ``{budget_code_key: [ {period_end, commitment_id, total_completed_and_stored_to_date,
    scheduled_value} ... ]}``. Mirrors :func:`load_owner_history`; enables as-of reconstruction of
    procore percent-complete (per-commitment latest row with period_end <= T, summed).
    """
    path = (
        Path(context_pkg)
        / "canonical"
        ...
        by_key[key].append(
            OrderedDict(
                [
                    ...
                    ("scheduled_value", r.get("scheduled_value")),
                ]
            )
        )
    for rows in by_key.values():
        rows.sort(key=lambda x: normalize_date(x["period_end"]) or "")
    return by_key
...
def build_signal_bundle(
    ...
    return OrderedDict(
        [
            ...
            (
                "actual_cost_through_may_2026",
                money_str(actuals.get("actual_cost_through_may_2026")),
            ),
            (
                "actual_cost_june_2026_to_date",
                money_str(actuals.get("actual_cost_june_2026_to_date")),
            ),
            ...
            (
                ...
                money_str(amounts.get("estimated_cost_at_completion")),
            ),
            ...
            (
                "schedule_remaining_duration_days",
                sched.get("remaining_duration_days") if sched else None,
            ),
            (
                "schedule_latest_remaining_finish",
                sched.get("latest_remaining_finish") if sched else None,
            ),
            (
