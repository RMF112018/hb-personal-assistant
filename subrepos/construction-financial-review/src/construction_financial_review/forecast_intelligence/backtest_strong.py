...
# next-gen method names scored historically (schedule has no history; excluded)
...
def _reconstruct(ctx: dict, owner_rows: list, target: Decimal) -> Optional[dict]:
    ...
    if remaining_months <= 0:
        return None
    return {
        "realized_final": realized, "actual_to_t": actual_to_t, "burn_to_t": burn_to_t,
        "owner_pct_to_t": asof_pct, "remaining_months": remaining_months, "t_month": t_month,
...
def run_strong_backtest(context_rows: list, owner_history: dict, project_key: str,
                        prior_summary: Optional[list] = None) -> dict:
    detail = []
        ...
        for target in ASOF_TARGETS:
            ...
            if preds:
                ...
                detail.append(OrderedDict([
                    ...
                    ("asof_target", str(target)), ("asof_month", m["t_month"]),
    ...
    for method in METHODS:
        ...
        summary.append(OrderedDict([
            ("method", method), ("n", len(errs[method])),
            ("mape", str(mape.quantize(Decimal("0.0001"))) if mape is not None else None),
            ("mean_bias", str((sum(biases[method], Decimal("0")) / Decimal(len(biases[method]))).quantize(Decimal("0.0001"))) if biases[method] else None),
        ]))
        if mape is not None:
    ...
    div_break = [OrderedDict([("division", d), ("n", len(v)),
                              ("mape", str(_mape(v).quantize(Decimal("0.0001"))))])
                 for d, v in sorted(div_errs.items()) if v]
    fam_break = [OrderedDict([("cost_code_family", f), ("n", len(v)),
                              ("mape", str(_mape(v).quantize(Decimal("0.0001"))))])
                 for f, v in sorted(fam_errs.items()) if v]
