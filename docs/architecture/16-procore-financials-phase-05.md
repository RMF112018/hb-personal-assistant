...
> **Current promotion status (machine-readable, Phase 06B Prompt 01).** The
> authoritative per-endpoint promotion ledger — promotion status, evidence path,
> last-verified date, and next step — is generated by `procore live endpoints
> ledger --json` and captured at
> `docs/evidence/construction-intelligence-phase-06b-procore-operational-intelligence/endpoint-promotion-ledger.json`.
