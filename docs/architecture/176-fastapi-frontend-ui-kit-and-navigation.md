...
- Prompt 22 (UI kit / a11y / responsive baseline): added reusable ErrorState (message + optional onRetry) and LoadingState under components/ui/; extended focus-visible to input/select/textarea + role selector; added skip link + <main id="main">; lightweight sidebar collapse (mobile drawer + md:static, Menu toggle with aria, overlay); Settings inline errors consolidated to ErrorState + explicit labels/aria for key inputs (output folder, pattern, stale, keywords) and confirmed checkbox/platform associations (FPR-011/013). Cross-ref prompt-22-ui-kit-accessibility-responsiveness-closeout.md and the Prompt 22 append in 00_PREFLIGHT.md. (Light update; 177 cross-ref if screen-level impact noted.)
- Prompt 23 (end-to-end local smoke harness, FPR-012/018): added Vitest + React Testing Library + jsdom harness (vitest.config, src/test/setup, "test"/"smoke:frontend" scripts) with component/adapter tests for the P22 ErrorState/LoadingState primitives (plus the spirit of protecting prior object-envelope contract fixes). Packaged repeatable scripted smoke via scripts/smoke_local.py (+ thin .sh wrapper) that exercises the exact UI-facing backend surfaces the pages query (today family, projects/all tabs, my-items, settings, admin with role, daily-brief status, health), asserts envelope shapes + no raw on the sensitive read models, drives frontend build + vitest, and fails on 404s or bad shapes. The full visual two-terminal (uvicorn 8000 + npm run dev 5173) per 06_VALIDATION_MATRIX is referenced in the closeout. Cross-ref prompt-23-end-to-end-local-smoke-harness-closeout.md and the Prompt 23 append in 00_PREFLIGHT.md. (Light update.)
- Prompt 24 (local-first production hardening, FPR-014/016): added app-level ErrorBoundary (class with getDerivedStateFromError + componentDidCatch; console.error only; CM-friendly fallback using P22 ErrorState tokens + "All signals advisory" + Reload via window.location.reload(); wired in main.tsx around AppRouter). For FPR-014 added explicit synthetic fixtures (tests/fixtures/daily_brief_analytics/ with FAKE/SYNTHETIC markers only) + expanded tests/test_fastapi_analytics_daily_brief.py (copy-to-tmp, states/path-display/bounded content, safe-subset asserts, pre/post sha256 proof that committed original fixture files remain unchanged on disk — "no source file mutation"). FPR-016 documented (evidence-only) as already closed in P20 (real PathPolicy local JSON + test roundtrip + response note; per spec "when fixed, document and do not rework"). Strengthened frontend scans (new scripts/proofs/frontend_safety_scan.py + receipt) + plain npm install/lint/type/build proof (no --legacy) + env defaults/failure states documented (DEFAULT_PREFS/DEFAULT_CONFIG + 7 STATE_LABELS + _compute_state; already surfaced via effective config/state/label on preferences/daily-brief surfaces). Cross-ref prompt-24-local-first-production-hardening-closeout.md and the Prompt 24 append in 00_PREFLIGHT.md. (Light update.)
- Prompt 25 (documentation and runbook packaging, FPR-018 final): packaged the consumable runbook (`docs/runbooks/frontend-local-analytics-smoke.md`) + evidence index (`docs/evidence/frontend-production-readiness-implementation/INDEX.md`) so a new developer can launch and smoke the local command center from the repo docs (one-command scripted path via P23 harness, two-terminal visual per 07 checklist with roles + no expected 404s/console clean + /chat inaccessible, capture instructions, known limitations with FPR-015 charts deferred, guardrails re-statement). Updated root + frontend READMEs with links and honest "implemented vs planned" notes. Light cross-refs added here and in 177/170/178/179. Doc link/path checks + fresh-clone-style smoke simulation + final stale-claim grep performed. P24 dep met. Cross-ref prompt-25-documentation-runbook-packaging-closeout.md and the Prompt 25 append in 00_PREFLIGHT.md. (Light update.)
