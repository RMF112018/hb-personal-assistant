...
Prompt 20: /api/settings/preferences now real (local JSON under App Support with schema_version + safe load/save, like daily_brief); /api/settings/keywords surface + per-project keywords/* (list/add/patch/delete/explain) power the new guided keyword management UI (redacted explain). Settings UI (guided sections, no raw, state helper for Daily Brief, real prefs, keyword CRUD) documented in 177 + prompt-20 closeout.
