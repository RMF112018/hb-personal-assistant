#!/bin/sh
# NAS deploy — tool-surface freshness parity fix (post manifest-promote surface_stale)
# RUN ON THE NAS: sudo sh /tmp/hb-deploy-freshness-fix.sh
set -eu
...
case "$DEPLOY_SHA" in
  ""|REPLACE_WITH_LANDED_SHA|UNSET) die "set DEPLOY_SHA to landed commit SHA" ;;
esac
...
say "DONE — next: re-transfer 14-manifest-refresh.sh, promote manifest r3, run 15-manifest-verify-only.sh"
cat <<EOF
SUMMARY
