...
- Patch 6 evidence: `docs/evidence/procore-null-projection-patch6-bare-container-policy/20260619T090000Z/`
- Raw strict findings remain preserved. Patch 6 does not fix, erase, drop, rename, hide, migrate, or populate bare container columns.
- `bare_container_deprecated_covered_by_scalar_decomposition` is a reporting/audit-policy disposition only. It is not a physical schema deprecation or migration.
- `bare_container_deprecated_covered_by_scalar_decomposition_total`: `34`.
  - Non-custom Patch 4 covered containers: `31`.
  - Patch 5 custom-field covered containers: `3`; this is a subtype of the `34` total, not an additive count.
- `bare_container_partially_covered_scalar_source_absent`: `4`.
- `source_absent_for_specific_scalar_column`: `5` scalar leaves across the partially covered parent containers.
- `bare_container_child_table_or_entity_only_deferred`: `5`.
- `company_id_policy_deferred`: `4`.
- Budget Detail remains untouched: `4` dead convenience columns and `4` read-model schema artifacts remain no-action/documentation decisions.
- Legacy non-null bare-container warnings: `6`; these are preexisting values, not mapping candidates and not proof that whole-object projection is acceptable.
- High-confidence mapping candidates remain `0`.
- Projection-code repair candidates remain `0`.
- Date/datetime mapping candidates remain `0`.

## Decision Matrix

| Decision bucket | Count | Affected families / tables | Evidence basis | Schema decision | Next action |
|---|---:|---|---|---|---|
| Reporting-policy deprecated bare containers covered by scalar decomposition | 34 | Change events, commitments, daily logs, inspections, meetings, observations, owner contracts, projects, purchase orders, RFIs, submittals; includes `3` Patch 5 purchase-order custom-field containers as a subtype | Patch 4/Patch 5 copied-DB reset/replay proof reset only existing scalar decomposition columns and replayed endpoint-limited local projections. Source-backed scalar paths repopulated after reset; bare object/container columns were not reset, written, hidden, dropped, renamed, or migrated. | Treat parent bare containers as reporting-policy deprecated/no-action schema artifacts covered by existing scalar decomposition. Do not map whole objects into bare columns. Preexisting non-null bare values are legacy-data warnings, not mapping proof. | `no_action_existing_scalar_decomposition_verified`; consider later bare-column deprecation only through a separate schema cleanup patch. |
...
| Source-absent scalar leaves within partially covered containers | 5 | `procore_ep_daily_log_manpower.contact`, `procore_ep_inspections.location`, `procore_ep_observations.specification_section`, `procore_ep_submittals.location` | Patch 4 identified `5` scalar leaves without current non-empty source evidence. | Do not count these scalar leaves as covered. Do not guess alternate source paths. | `review_scalar_source_coverage_before_schema_decision`. |
