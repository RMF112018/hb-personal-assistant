...
Add local fixtures, evidence templates, and safe CI validation.

## Likely Files / Modules Touched

.github/workflows, tests/fixtures, scripts, docs/evidence

## Required Work

1. Add deterministic local fixtures for source records, parser excerpts, body mention rows, calendar rows, files, and action candidates.
2. Add a safe validation script if useful.
3. Add GitHub Actions workflow for local-only validation.
4. Ensure CI does not require Microsoft credentials or a real Obsidian vault.
5. Add evidence templates and update validation register.

## Validation Commands

CI workflow syntax review; local full validation; sensitive scan.
...
Safe CI exists and local deterministic evidence can be regenerated without Graph consent.
...
```text
ci: add local assistant validation workflow
```
