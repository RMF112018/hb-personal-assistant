# Claude MCP Context Packet Builder

## Objective

Build daily brief, meeting prep, project review, follow-up queue, and weekly action review packets for Claude via MCP.
...
- MCP resources/tools must be read-only or local-output-only.
- No arbitrary SQL, raw content, raw file access, or writeback tools.
- Every packet item must be source-linked and review-tier/confidence aware.
...
```text
Phase 10 18: Claude MCP Context Packet Builder
