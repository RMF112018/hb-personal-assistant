# Local Model Structured Output Client

## Objective

Implement schema-enforced local model calls with Pydantic validation, timeouts, fallback, redacted/hashing-only receipts, and fixture tests.
...
- Implement Ollama readiness without requiring a successful model generation.
- Support profile checks for `qwen3:14b`, optional `gpt-oss:20b`, optional `qwen3:30b`, and test/mock provider.
- Heavy profile use must be blocked unless explicitly enabled.
- Use structured output/schema validation for generation tasks.
...
```text
Phase 10 04: Local Model Structured Output Client
