    ...
    <div className={`space-y-4 ${className}`}>
      {actions && (
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          {/* spacer to preserve actions alignment when title block is no longer rendered; only rendered when actions are supplied (status row appears directly under chrome when no actions bar) */}
