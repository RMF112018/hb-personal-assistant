...
def _rfi(status: str, *, due: str = "2026-01-10", updated: str = "2026-01-01T00:00:00Z") -> list[dict]:
    return [{
        "id": 101, "number": "RFI-001", "subject": "Door schedule clarification",
        "status": status, "due_date": due, "assignee_id": 42, "updated_at": updated,
    }]
...
def _count(db: Path, table: str, **where: str) -> int:
    ...
    try:
        if where:
            ...
            return conn.execute(f"SELECT COUNT(*) FROM {table} WHERE {clause}", tuple(where.values())).fetchone()[0]
        return conn.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0]
    finally:
        conn.close()


def test_first_sync_records_state_snapshot_and_created_event(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert _count(db, "procore_live_record_change_events", record_key=_RECORD_KEY, change_category="record_created") == 1
...
def test_unchanged_resync_adds_no_duplicate_snapshot(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert _count(db, "procore_live_record_snapshots", record_key=_RECORD_KEY) == 1
    assert _count(db, "procore_live_record_change_events", record_key=_RECORD_KEY) == 1  # only record_created
    # latest-state upsert remains idempotent (still one row)
    assert count_procore_live_records(project_key="tropical", endpoint_id="rfis", db_path=db) == 1


def test_changed_resync_records_snapshot_change_and_timeline(monkeypatch: pytest.MonkeyPatch) -> None:
    _setup_env(monkeypatch)
    db = _db()
    _sync(db, _rfi("open"))
    ...
    assert _count(db, "procore_live_record_change_events", record_key=_RECORD_KEY, change_category="closed") == 1
    assert _count(db, "procore_record_timeline_events", record_key=_RECORD_KEY, event_type="closed") == 1
...
def test_recent_changes_query(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _sync(db, _rfi("open"))
    _sync(db, _rfi("closed", updated="2026-02-01T00:00:00Z"))
    changes = get_procore_changes(project_key="tropical", since_utc="2000-01-01T00:00:00Z", db_path=db)
    ...
    assert get_procore_changes(project_key="tropical", since_utc="2999-01-01T00:00:00Z", db_path=db) == []
