...
(the contract's required set is a subset check); Phase 10 adds two local-agent-family
entries, so the seed now carries 11 agents while still containing all 9 required ones.
"""
...
def test_registry_validates_against_contracts() -> None:
    ...
    assert report["enabled_count"] == 11
