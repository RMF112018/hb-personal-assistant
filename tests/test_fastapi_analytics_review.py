"""N8C-9 read-only review API surface: GET-only, bounded, redacted, no write/disposition route."""
...
def client(tmp_path: Path, monkeypatch):
    ...
    return TestClient(create_app(db_path=db)), rid, item["target_kind"], item["target_id"]


def test_routes_ok_and_safe(client) -> None:
    c, rid, tkind, tid = client
    for path in ("/api/assistant/review/items", f"/api/assistant/review/items/{rid}",
                 f"/api/assistant/review/items/{rid}/dispositions",
                 f"/api/assistant/review/effective-state/{tkind}/{tid}",
                 "/api/assistant/review/summary"):
...
def test_effective_state_reflects_disposition(client) -> None:
    c, _rid, tkind, tid = client
    body = c.get(f"/api/assistant/review/effective-state/{tkind}/{tid}").json()
    assert body["effective_states"][0]["effective_state"] == "accepted"


def test_dispositions_listed(client) -> None:
    c, rid, _tkind, _tid = client
    body = c.get(f"/api/assistant/review/items/{rid}/dispositions").json()
    assert body["count"] >= 1 and body["dispositions"][0]["disposition_type"] == "accept"


def test_missing_returns_404(client) -> None:
    c, _rid, _tkind, _tid = client
    assert c.get("/api/assistant/review/items/nope").status_code == 404
    assert c.get("/api/assistant/review/items/nope/dispositions").status_code == 404


def test_all_roles_allowed(client) -> None:
    c, _rid, _tkind, _tid = client
    for role in ("viewer", "operator", "admin"):
        assert c.get("/api/assistant/review/summary",
...
def test_routes_are_get_only(client) -> None:
    c, _rid, _tkind, _tid = client
    surface = {
        "/api/assistant/review/items", "/api/assistant/review/items/{review_item_id}",
        "/api/assistant/review/items/{review_item_id}/dispositions",
        "/api/assistant/review/effective-state/{target_kind}/{target_id}",
        "/api/assistant/review/summary",
    }
...
def test_no_write_or_disposition_route(client) -> None:
    c, rid, _tkind, _tid = client
    # no POST/PUT/PATCH/DELETE on the review surface, and specifically no disposition write route
    assert c.post("/api/assistant/review/items").status_code in {401, 404, 405}
    assert c.post(f"/api/assistant/review/items/{rid}/dispositions").status_code in {401, 404, 405}
    assert c.delete("/api/assistant/review/items").status_code in {401, 404, 405}


def test_bounded_limit_is_clamped(client) -> None:
    c, _rid, _tkind, _tid = client
    r = c.get("/api/assistant/review/items?limit=100000", headers={"X-HB-UI-Role": "viewer"})
    assert r.status_code == 200
    assert len(r.json()["review_items"]) <= 200
