...
def _seed(store: ConstructionStore, db: str) -> None:
    for key, document_type in [("k1", "rfi"), ("k2", "submittal"), ("k3", "drawings")]:
        ...
        store.upsert_document_card(
            ...
            file_extension="pdf", project_key="alpha",
        )
    ...
    for endpoint_id, rid in [("rfis", "1"), ("submittals", "2")]:
        ...
        record_sync_run_start(
            ...
            legacy_endpoint_alias=None, project_key="alpha", procore_project_id="pp1",
            company_id="co1", mode="history", started_at_utc="2026-01-01T00:00:00Z", db_path=Path(db),
        )
        upsert_procore_live_record(
            project_key="alpha", procore_project_id="pp1", endpoint_id=endpoint_id,
            procore_record_id=rid, parent_procore_id=None, normalized_fields={"number": rid},
            review_required=False, sensitive_reason=None, source_url_redacted=None,
            last_sync_run_id=run_id, now_utc="2026-01-01T00:00:00Z", db_path=Path(db),
        )
