...
_INSPECTION = {
    "id": 777, "identifier": "INS-1", "number": 1, "name": "Jobsite Safety Checklist", "status": "Open",
    "private": True, "overdue": False, "closed_at": None, "respondable_item_count": 32,
    "inspected_item_count": 0, "deficient_item_count": 2, "observations_count": 0, "item_count": 32,
    "inspection_type": {"id": 5, "name": "Safety"}, "list_template_id": 9, "list_template_name": "Safety List",
...
_RESPONSE_SET = {
    "id": 50, "name": "Pass/Fail", "active": True, "procore_standard": True, "updated_at": _NOW,
    "responses": [
...
def _item(responded_with: str) -> dict:
    return {
        "id": 1251207033, "name": "Fall Exposures", "number": "1.1", "list_id": 777, "section_id": 10,
        "status": "not_started", "responded_with": responded_with, "parent_item_id": None,
        "template_item_id": 3, "position": 1, "relative_position": 1, "updated_at": _NOW,
...
def test_inspection_record_projection_and_signals() -> None:
    db = _db()
    project_inspection_record(_INSPECTION, project_key="tropical", sync_run_id="r1", now_utc=_NOW, db_path=db)
    ...
    sigs = _signals(db)
    assert {"inspection_open_safety", "inspection_has_deficient_items", "inspection_has_unanswered_items"} <= sigs
...
def test_section_risk_categorization() -> None:
    db = _db()
    project_inspection_section(
        {"id": 10, "name": "Areas of Highest Risk", "position": 1, "template_section_id": 99, "updated_at": _NOW},
        project_key="tropical", now_utc=_NOW, db_path=db,
    )
    row = _conn(db).execute("SELECT name_redacted, risk_category FROM procore_inspection_sections").fetchone()
...
def test_section_low_risk_categorization() -> None:
    ...
    row = _conn(db).execute("SELECT risk_category FROM procore_inspection_sections WHERE section_id='11'").fetchone()
...
def test_item_no_response_projection_and_signal() -> None:
    db = _db()
    out = project_inspection_item(_item("No Response"), project_key="tropical", sync_run_id="r1", now_utc=_NOW, db_path=db)
...
def test_item_fail_is_deficient_and_non_conforming() -> None:
    db = _db()
    project_inspection_item(_item("Fail"), project_key="tropical", sync_run_id="r1", now_utc=_NOW, db_path=db)
    row = _conn(db).execute("SELECT is_deficient, is_conforming, response_status FROM procore_inspection_items").fetchone()
    assert row["is_deficient"] == 1 and row["is_conforming"] == 0 and row["response_status"] == "deficient"
    sigs = _signals(db)
...
def test_item_pass_is_conforming() -> None:
    db = _db()
    project_inspection_item(_item("Pass"), project_key="tropical", sync_run_id="r1", now_utc=_NOW, db_path=db)
    row = _conn(db).execute("SELECT is_conforming, is_unanswered FROM procore_inspection_items").fetchone()
    assert row["is_conforming"] == 1 and row["is_unanswered"] == 0


def test_response_set_and_options_projection() -> None:
    db = _db()
    project_inspection_item(_item("No Response"), project_key="tropical", sync_run_id="r1", now_utc=_NOW, db_path=db)
    sets = _conn(db).execute("SELECT * FROM procore_inspection_response_sets").fetchall()
    opts = _conn(db).execute("SELECT name_redacted, status_category FROM procore_inspection_response_options ORDER BY response_option_id").fetchall()
    assert len(sets) == 1 and sets[0]["name_redacted"] == "Pass/Fail"
    assert {(o["name_redacted"], o["status_category"]) for o in opts} == {
        ("Pass", "conforming"), ("Fail", "deficient"), ("No Response", ""),
    }
...
def test_evidence_rules_projection_and_signals() -> None:
    db = _db()
    project_inspection_item(_item("No Response"), project_key="tropical", sync_run_id="r1", now_utc=_NOW, db_path=db)
    row = _conn(db).execute("SELECT requires_observation, requires_photo, photo_response_option_ids_json FROM procore_inspection_evidence_rules").fetchone()
    assert row["requires_observation"] == 1 and row["requires_photo"] == 1
    assert "2" in (row["photo_response_option_ids_json"] or "")
    sigs = _signals(db)
    assert {"inspection_item_requires_photo_evidence", "inspection_item_requires_observation"} <= sigs


def test_projection_is_idempotent() -> None:
    db = _db()
    project_inspection_item(_item("No Response"), project_key="tropical", sync_run_id="r1", now_utc=_NOW, db_path=db)
    project_inspection_item(_item("No Response"), project_key="tropical", sync_run_id="r2", now_utc=_NOW, db_path=db)
    assert _conn(db).execute("SELECT COUNT(*) FROM procore_inspection_items").fetchone()[0] == 1
    assert _conn(db).execute("SELECT COUNT(*) FROM procore_inspection_response_options").fetchone()[0] == 3
...
def test_item_response_change_records_history_and_projection(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _sync(responded_with: str) -> None:
        run_live_sync(
            project_key="tropical", endpoint="inspection-items", apply=True, sqlite_only=True,
