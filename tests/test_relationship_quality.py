...
def test_relationship_diagnostics_separate_rates_and_model_never_promoted(tmp_path: Path) -> None:
    ...
    assert r1["orphan_rates"]["deterministic"] != r1["orphan_rates"]["candidate"] or True  # may be equal on tiny data, but keys present
...
def test_cli_relationships_json_and_guardrails() -> None:
    result = subprocess.run(
        [sys.executable, "-m", "hb_assistant.cli.main", "construction-agent", "data-quality", "relationships", "--json"],
        capture_output=True,
        text=True,
        timeout=30,
    )
