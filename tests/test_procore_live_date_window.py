...
def _run(
    endpoint: str, transport: Any, monkeypatch: pytest.MonkeyPatch, **kw: Any
) -> Dict[str, Any]:
...
def test_date_window_passed_as_query_params(monkeypatch: pytest.MonkeyPatch) -> None:
    transport = _CapturingTransport(
        [{"id": 1, "date": "2026-03-15", "num_workers": 4, "man_hours": 32}]
    )
    receipt = _run(
        "daily-log-manpower", transport, monkeypatch, start_date="2026-01-01", end_date="2026-05-29"
    )
