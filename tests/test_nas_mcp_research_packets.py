"""N8C-11 — remote NAS MCP read-only research-packet tools: safety + behavior.

Proves the six remote tools return bounded, citation-backed data from a READ-ONLY DB snapshot
(``mode=ro&immutable=1`` + ``query_only=ON``) and never write / build / generate an answer; are gated by a
default-ON kill switch (``HB_MCP_ASSISTANT_RESEARCH_PACKETS``) that disables ONLY the research-packet tools;
preserve the existing nav (12) + context-pack (4) + memory (4) + decision-memory (6) + review (5) +
intelligence (5) tools BY NAME; add NO build/apply/answer/action tool; and keep ``ai_outputs_card_upsert``
the ONLY sanctioned remote write.
"""
...
from hb_assistant.obsidian_mcp.research_packet_models import REVIEW_AWARE_ANSWER_CONTEXT
...
def mcp_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    proj = IB.build_intelligence_projection(
        IB.ProjectionProviders(RB.ReviewProviders(pr, cr, er, sr, MemoryRepository(db), dr), rrepo),
        irepo, pack_id=pack, projection_type=REVIEW_AWARE_CONTEXT, apply=True)["projection_id"]
    res = PB.build_research_packet(PB.PacketProviders(irepo), ResearchPacketRepository(db),
                                   projection_id=proj, packet_type=REVIEW_AWARE_ANSWER_CONTEXT, apply=True)
    ...
    return {"broker": NasMcpBroker(cfg), "db": db, "packet_id": res["packet_id"]}
...
def test_tools_return_data(mcp_env) -> None:
    b, pid = mcp_env["broker"], mcp_env["packet_id"]
    assert _ok(b, "assistant_list_research_packets", {})["count"] >= 1
    assert _ok(b, "assistant_get_research_packet", {"packet_id": pid})["packet"]["packet_id"] == pid
    assert "items" in _ok(b, "assistant_get_research_packet_items", {"packet_id": pid})
    assert "citations" in _ok(b, "assistant_get_research_packet_citations", {"packet_id": pid})
    assert "answer_contract" in _ok(b, "assistant_get_research_packet_export", {"packet_id": pid})
    assert "summary" in _ok(b, "assistant_get_research_packet_summary", {})


def test_missing_denied_cleanly(mcp_env) -> None:
    r = mcp_env["broker"].dispatch("assistant_get_research_packet", {"packet_id": "nope"})
    assert r["ok"] is False and "packet_not_found" in r["error"]
...
def test_snapshot_is_read_only(mcp_env) -> None:
    ...
    try:
        with pytest.raises(sqlite3.OperationalError):
            conn.execute("UPDATE assistant_research_packets SET status='stale'")
    finally:
        conn.close()


def test_kill_switch_disables_only_research_packets(mcp_env, monkeypatch: pytest.MonkeyPatch) -> None:
    b = mcp_env["broker"]
    assert b.dispatch("assistant_list_research_packets", {})["ok"] is True  # default ON
    monkeypatch.setenv("HB_MCP_ASSISTANT_RESEARCH_PACKETS", "0")
    off = b.dispatch("assistant_list_research_packets", {})
    assert off["ok"] is False and off["error"] == "assistant_research_packets_disabled"
    # sibling surfaces stay enabled — the kill switch is scoped to research packets only
...
def test_no_write_build_answer_or_action_tool_registered(mcp_env) -> None:
    mcp = _FakeMcp()
    ...
    assert set(ASSISTANT_RESEARCH_PACKET_TOOLS) <= set(assistant)
    ...
    assert len(ASSISTANT_RESEARCH_PACKET_TOOLS) == 6


def test_status_reports_research_packets(mcp_env) -> None:
    res = mcp_env["broker"].dispatch("hb_mcp_status", {})["result"]
    assert res["assistant_research_packets_enabled"] is True
    assert set(res["assistant_research_packet_tools"]) == set(ASSISTANT_RESEARCH_PACKET_TOOLS)
    # ai_outputs_card_upsert remains the only sanctioned write.
    assert "ai_outputs_card_upsert" not in set(ASSISTANT_RESEARCH_PACKET_TOOLS)
