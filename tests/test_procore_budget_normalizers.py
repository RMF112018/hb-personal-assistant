...
def test_budget_detail_column_keeps_definition_fields() -> None:
    cf = normalize_budget_detail_column(
        {"id": "col_1", "name": "Projected Costs", "type": "currency", "position": 3,
         "aggregatable": True, "filterable": True, "groupable": False},
...
def test_budget_detail_row_amounts_and_hashed_text() -> None:
    ...
    cf = normalize_budget_detail_row(raw, endpoint_id="budget-detail-rows", **_KW)["canonical_fields"]
    blob = json.dumps(cf)
...
def test_budget_change_history_old_new_amounts() -> None:
    cf = normalize_budget_change_history(
        {"budget_code": "01-100", "column": "Revised Budget", "type": "adjustment",
         "old_value": "100.00", "new_value": "150.50", "description": "bump",
         "created_at": "2026-05-20", "created_by": {"id": 5, "name": "Pat"}},
...
def test_budget_modification_transfer_amount() -> None:
    cf = normalize_budget_modification(
        {"id": 11, "from_budget_line_item_id": 100, "to_budget_line_item_id": 200,
         "transfer_amount": "2500.00", "notes": "reallocate per pm@example.test",
