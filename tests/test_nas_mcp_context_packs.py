"""N8C-6 — remote NAS MCP read-only enrichment-review + context-pack tools: safety + behavior.

Proves the four remote tools:
  * return bounded, source-linked data from a READ-ONLY DB snapshot (``query_only``, no live-DB
    fallback) and never write;
  * are gated by a default-ON kill switch (``HB_MCP_ASSISTANT_CONTEXT_PACKS``), independent of the
    write gates;
  * preserve the existing 12 ``assistant_*`` nav tools and keep ``ai_outputs_card_upsert`` the ONLY
    sanctioned remote write — no build/apply/write context-pack tool is exposed.
"""
...
def mcp_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    return {"broker": NasMcpBroker(cfg), "db": db, "pack_id": res["pack_id"]}
...
def test_context_pack_tools_return_data(mcp_env) -> None:
    b, pid = mcp_env["broker"], mcp_env["pack_id"]
    assert _ok(b, "assistant_list_context_packs", {})["count"] == 1
    assert _ok(b, "assistant_get_context_pack", {"pack_id": pid})["context_pack"]["pack_id"] == pid
    assert "items" in _ok(b, "assistant_get_context_pack_items", {"pack_id": pid})
    assert "review_items" in _ok(b, "assistant_list_enrichment_review_items", {})


def test_missing_pack_denied_cleanly(mcp_env) -> None:
    r = mcp_env["broker"].dispatch("assistant_get_context_pack", {"pack_id": "nope"})
    assert r["ok"] is False and "context_pack_not_found" in r["error"]
...
def test_snapshot_is_read_only(mcp_env) -> None:
    ...
    try:
        with pytest.raises(sqlite3.OperationalError):
            conn.execute("UPDATE assistant_context_packs SET status='stale'")
    finally:
        conn.close()
...
def test_kill_switch(mcp_env, monkeypatch: pytest.MonkeyPatch) -> None:
    b = mcp_env["broker"]
    assert b.dispatch("assistant_list_context_packs", {})["ok"] is True   # default ON
    monkeypatch.setenv("HB_MCP_ASSISTANT_CONTEXT_PACKS", "0")
    off = b.dispatch("assistant_list_context_packs", {})
    assert off["ok"] is False and off["error"] == "assistant_context_packs_disabled"


def test_reads_are_not_writes_safe_mode(mcp_env, monkeypatch: pytest.MonkeyPatch) -> None:
    b = mcp_env["broker"]
    monkeypatch.setenv("HB_MCP_SAFE_MODE", "1")
    assert b.dispatch("assistant_list_context_packs", {})["ok"] is True  # reads survive safe mode
...
def test_no_context_pack_write_tool_registered(mcp_env) -> None:
    ...
    assert not [n for n in assistant if any(v in n for v in ("build", "apply", "write", "create",
                                                             "delete", "persist", "upsert"))]
    assert len(ASSISTANT_CONTEXT_PACK_TOOLS) == 4


def test_status_reports_context_packs(mcp_env) -> None:
    res = mcp_env["broker"].dispatch("hb_mcp_status", {})["result"]
    assert res["assistant_context_packs_enabled"] is True
    assert set(res["assistant_context_pack_tools"]) == set(ASSISTANT_CONTEXT_PACK_TOOLS)
