"""N8C-8 — remote NAS MCP read-only decision/preference/open-loop tools: safety + behavior.

Proves the six remote tools:
  ...
  * are gated by a default-ON kill switch (``HB_MCP_ASSISTANT_DECISION_MEMORY``), independent of the
    write gates;
  * preserve the existing 12 nav + 4 context-pack + 4 memory tools, add NO extract/apply/write/action
    tool, and keep ``ai_outputs_card_upsert`` the ONLY sanctioned remote write.
"""
...
def mcp_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    return {"broker": NasMcpBroker(cfg), "db": db, "decision_id": did, "open_loop_id": oid}
...
def test_tools_return_data(mcp_env) -> None:
    ...
    assert _ok(b, "assistant_get_decision", {"decision_id": did})["decision"]["decision_id"] == did
    assert "preferences" in _ok(b, "assistant_list_preferences", {})
    assert _ok(b, "assistant_list_open_loops", {})["count"] >= 1
...
def test_snapshot_is_read_only(mcp_env) -> None:
    ...
    try:
        with pytest.raises(sqlite3.OperationalError):
            conn.execute("UPDATE assistant_decision_records SET status='stale'")
    finally:
        conn.close()
...
def test_kill_switch(mcp_env, monkeypatch: pytest.MonkeyPatch) -> None:
    b = mcp_env["broker"]
    assert b.dispatch("assistant_list_decisions", {})["ok"] is True   # default ON
    monkeypatch.setenv("HB_MCP_ASSISTANT_DECISION_MEMORY", "0")
    off = b.dispatch("assistant_list_decisions", {})
    assert off["ok"] is False and off["error"] == "assistant_decision_memory_disabled"


def test_reads_are_not_writes_safe_mode(mcp_env, monkeypatch: pytest.MonkeyPatch) -> None:
    b = mcp_env["broker"]
    monkeypatch.setenv("HB_MCP_SAFE_MODE", "1")
    assert b.dispatch("assistant_list_decisions", {})["ok"] is True  # reads survive safe mode
...
def test_no_write_or_action_tool_registered(mcp_env) -> None:
    ...
    # nav (12) + context-pack (4) + memory (4) + decision (6) preserved; nothing implying a
    # write/extract/apply/close/action.
    ...
    assert not [n for n in assistant if any(v in n for v in (
        ...
        "accept", "reject", "mark", "send", "remind"))]
    assert len(ASSISTANT_DECISION_MEMORY_TOOLS) == 6


def test_status_reports_decision_memory(mcp_env) -> None:
    res = mcp_env["broker"].dispatch("hb_mcp_status", {})["result"]
    assert res["assistant_decision_memory_enabled"] is True
    assert set(res["assistant_decision_memory_tools"]) == set(ASSISTANT_DECISION_MEMORY_TOOLS)
