...
def test_review_policy_status_runs_and_loads_packaged():
    ...
    assert result.exit_code in (
        0,
        3,
    )  # 3 if policy not fully satisfied in this env, but must not crash
    assert (
        "policy" in (result.stdout or "")
        or "review_burden" in (result.stdout or "")
        ...
        for r in (r1, r3):
            if r.exit_code == 0 and r.stdout:
                try:
                    ...
                    for c in (data.get("mart", {}).get("clusters") or data.get("clusters") or [])[
                        :2
                    ]:
                        if "item_count" in c:
                            ...
                            hashes = [
                                (e.get("item_hash") or e.get("source_ref_hash")) for e in tops
                            ]
