...
run-output tables present and empty; the lifecycle contract stays self-consistent and the
ten tables classify as the v63 run-output family. Additive schema only — no network, no CFR.
"""
...
def test_lifecycle_contract_count_and_v63_classification() -> None:
    ...
    # Self-consistency rather than a brittle absolute: the run-output family adds 10 tables,
    # but the running total moves as other migrations land alongside it.
