"""N8C-11 read-only research-packet API surface: GET-only, bounded, citation-backed, redacted, no write/
build/answer route."""
...
def client(tmp_path: Path, monkeypatch):
    ...
    return TestClient(create_app(db_path=db)), res["packet_id"]


def test_routes_ok_and_safe(client) -> None:
    c, pid = client
    for path in ("/api/assistant/research-packets",
                 f"/api/assistant/research-packets/{pid}",
                 f"/api/assistant/research-packets/{pid}/items",
                 f"/api/assistant/research-packets/{pid}/citations",
                 f"/api/assistant/research-packets/{pid}/export",
                 "/api/assistant/research-packets/summary"):
...
def test_export_carries_answer_contract(client) -> None:
    c, pid = client
    body = c.get(f"/api/assistant/research-packets/{pid}/export",
                 headers={"X-HB-UI-Role": "viewer"}).json()
    ac = body["answer_contract"]
    assert ac["citation_required"] is True and ac["action_policy"] == "no_execution"
    assert "answer_allowed" in ac


def test_missing_returns_404(client) -> None:
    c, _pid = client
    assert c.get("/api/assistant/research-packets/nope").status_code == 404
    assert c.get("/api/assistant/research-packets/nope/items").status_code == 404
    assert c.get("/api/assistant/research-packets/nope/citations").status_code == 404
    assert c.get("/api/assistant/research-packets/nope/export").status_code == 404


def test_all_roles_allowed(client) -> None:
    c, _pid = client
    for role in ("viewer", "operator", "admin"):
        assert c.get("/api/assistant/research-packets/summary",
...
def test_routes_are_get_only(client) -> None:
    c, _pid = client
    surface = {
        "/api/assistant/research-packets",
        "/api/assistant/research-packets/{packet_id}",
        "/api/assistant/research-packets/{packet_id}/items",
        "/api/assistant/research-packets/{packet_id}/citations",
        "/api/assistant/research-packets/{packet_id}/export",
        "/api/assistant/research-packets/summary",
    }
...
def test_no_write_or_build_route(client) -> None:
    c, pid = client
    assert c.post("/api/assistant/research-packets").status_code in {401, 404, 405}
    assert c.post(f"/api/assistant/research-packets/{pid}/items").status_code in {401, 404, 405}
    assert c.delete("/api/assistant/research-packets").status_code in {401, 404, 405}


def test_bounded_limit_is_clamped(client) -> None:
    c, pid = client
    r = c.get(f"/api/assistant/research-packets/{pid}/citations?limit=100000",
