...
_RFI: Dict[str, Any] = {
    "id": 501, "number": "RFI-001", "subject": "Door schedule", "status": "Open",
    "translated_status": "Open", "due_date": "2026-02-01", "full_number": "RFI-001",
    "cost_impact": {"status": "yes", "value": None}, "schedule_impact": {"status": "no_impact", "value": None},
    ...
    "replies": [{
        "id": 9001, "plain_text_body": "Official answer: proceed as drawn.", "official": True,
        "created_by_id": 77,
        "attachments": [{"id": 7, "filename": "a.pdf", "url": "https://example.test/f/abc?token=secret"}],
    }],
}
...
def test_rfi_people_company_edges_hashed() -> None:
    ...
    assert {"received_from", "rfi_manager", "assignee", "ball_in_court", "responsible_contractor"} <= edges
...
def test_rfi_question_text_intelligence_encrypted() -> None:
    ...
    row = _conn(db).execute(
        "SELECT * FROM procore_text_intelligence WHERE source_field_path='question'"
    ).fetchone()
...
def test_rfi_response_official_answer_and_edge() -> None:
    ...
    assert resp_text and decrypt_text(resp_text["encrypted_full_text_ref"]) == "Official answer: proceed as drawn."
...
def test_rfi_projection_idempotent() -> None:
    ...
    assert c.execute("SELECT COUNT(*) FROM procore_text_intelligence WHERE source_field_path='question'").fetchone()[0] == 1


def test_response_projection_direct_without_parent() -> None:
    db = _db()
    out = project_rfi_response(
        {"id": 9, "rich_text_body": "<b>answer</b>", "official": False, "created_by_id": 5, "attachments": []},
        parent_rfi_id=None, project_key="tropical", now_utc=_NOW, db_path=db,
    )
...
def _rfi_payload(bic_id: int) -> list[dict]:
    return [{
        "id": 700, "number": "RFI-700", "subject": "Footing depth", "status": "Open",
...
def test_ball_in_court_change_event_and_signal(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _sync(bic_id: int) -> None:
        run_live_sync(
            project_key="tropical", endpoint="rfis", apply=True, sqlite_only=True,
