...
def test_run_state_disabled_by_config(tmp_path):
    ...
    _bootstrap(db, ocfg, acfg, all_roots=True)
    assert sb.resolve_run_state("k", db_path=db, obsidian_config=ocfg, backend_available=True) == (
...
def test_run_state_backend_unavailable(tmp_path):
    db, _root, ocfg, acfg = _make_env(tmp_path)
    _bootstrap(db, ocfg, acfg, all_roots=True)
    assert sb.resolve_run_state("k", db_path=db, obsidian_config=ocfg, backend_available=False) == (
