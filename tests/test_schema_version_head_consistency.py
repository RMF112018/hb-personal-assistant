...
def test_v105_migration_row_present(tmp_path: Path) -> None:
    db = tmp_path / "head.db"
    _migrate(db)
    with sqlite3.connect(db) as conn:
        row = conn.execute("SELECT name FROM schema_migrations WHERE version = 105").fetchone()
        tables = {
            r[0]
            for r in conn.execute(
                "SELECT name FROM sqlite_master WHERE type='table' "
                "AND name LIKE 'assistant_review%'"
            )
        }
    assert row is not None
    assert row[0] == "v105_assistant_review_queue"
    assert tables == {
...
def test_prior_assistant_tables_survive_v105(tmp_path: Path) -> None:
    # V105 is additive: V100 claim, V101 enrichment, V102 context-pack, V103 memory, V104
    # decision-memory tables must remain.
    db = tmp_path / "head.db"
    _migrate(db)
    with sqlite3.connect(db) as conn:
        names = {
...
def test_v106_migration_row_present(tmp_path: Path) -> None:
    db = tmp_path / "head.db"
    _migrate(db)
    with sqlite3.connect(db) as conn:
        row = conn.execute("SELECT name FROM schema_migrations WHERE version = 106").fetchone()
        tables = {
            r[0]
            for r in conn.execute(
                "SELECT name FROM sqlite_master WHERE type='table' "
                "AND name LIKE 'assistant_intelligence_projection%'"
            )
        }
    assert row is not None
    assert row[0] == "v106_assistant_intelligence_projection"
    assert tables == {
...
def test_prior_assistant_tables_survive_v106(tmp_path: Path) -> None:
    # V106 is additive: V100–V105 tables (claim / enrichment / context-pack / memory / decision-memory /
    # review-overlay) must remain.
    db = tmp_path / "head.db"
    _migrate(db)
    with sqlite3.connect(db) as conn:
        names = {
