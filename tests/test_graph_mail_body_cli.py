...
def _invoke(tmp_path: Path, *args: str):
    return runner.invoke(app, list(args), env={"HB_APP_SUPPORT_DIR": str(tmp_path)}, catch_exceptions=False)
...
def _seed_body_ref() -> str:
    ...
    store.upsert_email_body_vault_ref(
        message_id="m1", encrypted_full_body_ref=ref or "r", body_hash="a" * 64,
        body_length=len(_SYNTHETIC), extraction_policy="encrypted_text_vault",
        body_content_type="text", review_required=False, sensitivity_classification=None,
    )
...
def test_body_show_unknown_id_found_false(tmp_path: Path) -> None:
    res = _invoke(tmp_path, "graph", "mail", "body", "show", "--message-id", "nope", "--reason", "validation", "--json")
...
def test_body_show_redacted_summary_no_plaintext() -> None:
    ...
    res = runner_local.invoke(
        app,
        ["graph", "mail", "body", "show", "--message-id", "m1", "--reason", "operator_review", "--json"],
