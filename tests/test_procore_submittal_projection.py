            ...
            "attachments": [
                {
                    "id": 7,
                    "filename": "markup.pdf",
                    "url": "https://example.test/f/abc?token=secret",
                }
            ],
            "attachment_ids": [8],
        },
...
def test_submittal_approver_extraction_hashed_with_edges() -> None:
    db = _db()
    project_submittal(
        _SUBMITTAL, project_key="tropical", sync_run_id="r1", now_utc=_NOW, db_path=db
    )
    c = _conn(db)
...
def test_submittal_workflow_duration_metrics_on_approver_edge() -> None:
    ...
    rows = (
        _conn(db)
        .execute("SELECT metadata_json FROM procore_record_edges WHERE edge_type='approver'")
        .fetchall()
    )
...
def test_submittal_response_interpretation_and_returned_signal() -> None:
    ...
    resp = (
        _conn(db)
        .execute("SELECT * FROM procore_text_intelligence WHERE source_field_path='comment'")
        .fetchone()
    )
    assert (
        resp is not None
        and decrypt_text(resp["encrypted_full_text_ref"]) == "Proceed as annotated."
    )
...
def test_submittal_required_on_site_and_waiting_signals() -> None:
    ...
    assert {
        "submittal_open",
        "submittal_overdue",
        "submittal_waiting_on_approver",
        "submittal_required_on_site_date_near",
    } <= sigs
...
def test_submittal_projection_idempotent() -> None:
    ...
    project_submittal(
        _SUBMITTAL, project_key="tropical", sync_run_id="r2", now_utc=_NOW, db_path=db
    )
    c = _conn(db)
