...
_DETAIL = {
    "id": 555, "title": "Weekly Coordination", "created_by_id": 160586,
    ...
    "attendees": [
        {"id": 1, "status": None, "login_information": {"id": 9, "login": "alice@example.test", "name": "Alice A"}},
    ],
    "attachments": [{"id": 7, "filename": "agenda.pdf", "url": "https://example.test/f/abc?token=secret"}],
    "meeting_categories": [{
        "id": 10, "title": "Open Items", "position": 1,
        "meeting_topic": [{
            "id": 1001, "title": "Fall protection", "number": "1.1", "status": "open", "priority": "high",
            "description": "Need permit and utilities coordination. RFI 123 pending.", "minutes": None,
            "attachments": [{"id": 8, "filename": "photo.jpg", "url": "https://example.test/p/xyz?company_id=15"}],
        }],
    }],
}
...
def test_meeting_detail_attendees_categories_topics() -> None:
    db = _db()
    project_meeting_detail(_DETAIL, project_key="tropical", sync_run_id="r1", now_utc=_NOW, db_path=db)
    c = _conn(db)
...
def test_meeting_series_edge() -> None:
    db = _db()
    project_meeting({"id": 200, "parent_id": 100, "title": "Mtg #2"}, project_key="tropical", now_utc=_NOW, db_path=db)
...
def test_projection_idempotent() -> None:
    db = _db()
    project_meeting_detail(_DETAIL, project_key="tropical", sync_run_id="r1", now_utc=_NOW, db_path=db)
    project_meeting_detail(_DETAIL, project_key="tropical", sync_run_id="r2", now_utc=_NOW, db_path=db)
    c = _conn(db)
    assert c.execute("SELECT COUNT(*) FROM procore_attachment_refs").fetchone()[0] == 2
    assert c.execute("SELECT COUNT(*) FROM procore_text_intelligence WHERE source_field_path='description'").fetchone()[0] == 2
...
def test_grouped_meetings_flattening_and_series_edge(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    grouped = [{
        "group_title": "May",
        "meetings": [
            {"id": 100, "title": "Kickoff", "starts_at": "2026-05-01T00:00:00Z", "updated_at": "2026-05-01T00:00:00Z"},
            {"id": 200, "title": "Follow-up", "parent_id": 100, "starts_at": "2026-05-08T00:00:00Z", "updated_at": "2026-05-08T00:00:00Z"},
        ],
    }]
    run_live_sync(
        project_key="tropical", endpoint="meetings", apply=True, sqlite_only=True,
        confirm_live_get=True, max_pages=1, max_items=10, db_path=db, transport=_FakeTransport(grouped),
    )
    c = _conn(db)
    # both grouped meetings were flattened + persisted as latest-state rows
    assert c.execute("SELECT COUNT(*) FROM procore_live_records WHERE endpoint_id='meetings'").fetchone()[0] == 2
