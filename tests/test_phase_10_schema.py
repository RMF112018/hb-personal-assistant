"""Phase 10 / 10A schema migration + status tests (V41 + V42 raw addendum).

Covers: V41/V42 migrations apply and are idempotent (prior versions preserved); V41 tables have
guards; V42 raw tables exist (no guards by design); raw tables row counts 0 on fresh; stale fail-closed;
prior metadata tables untouched; status report surfaces raw counts when >=V42.
...
def test_migration_applies_v41_with_all_tables() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        # Phase 10A Prompt 02: raw content tables (V42) present, 6 tables, row counts start at 0.
        raw_missing = [t for t in PHASE_10A_RAW_TABLES if t not in names]
        assert raw_missing == []
        assert len(PHASE_10A_RAW_TABLES) == 6
        conn = sqlite3.connect(db)
        for t in PHASE_10A_RAW_TABLES:
...
def test_migration_idempotent_and_preserves_prior_versions() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        n42 = conn.execute("SELECT COUNT(*) FROM schema_migrations WHERE version=42").fetchone()[0]
        assert n41 == 1
        assert n42 == 1
        names = {r[0] for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'")}
        conn.close()
        # V40 + a Phase 09 (V38/39) table still present — additive only. No raw tables affect them.
        assert "construction_project_keyword_registry" in names
        assert PHASE_09_V38_TABLES[0] in names
        # V42 raw tables also present after double apply.
        for t in PHASE_10A_RAW_TABLES:
            assert t in names
        # Explicit guard: a core metadata table from pre-10A (e.g. email_messages or source_records) untouched.
        # We assert a known long-standing table from V1 is still there.
...
def test_schema_status_ready() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        # Phase 10A: raw tables surfaced with counts 0, no guard claims.
        assert "raw_content_tables" in report
        assert report.get("raw_content_table_count") == 6
        raw_names = [t["table_name"] for t in report.get("raw_content_tables", [])]
        assert set(raw_names) == set(PHASE_10A_RAW_TABLES)
        for t in report.get("raw_content_tables", []):
