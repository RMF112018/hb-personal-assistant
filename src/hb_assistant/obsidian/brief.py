    ...
    def generate_for_date(
        ...
        for hit in (context.retrieved or [])[:8]:
            ...
            if "waiting" in excerpt.lower():
                ...
                waiting_on.append(
                    f"- [ ] [signal] {excerpt[:90]}{'...' if len(excerpt) > 90 else ''} (src={sid})"
                )
        ...
        for hit in (context.retrieved or [])[:8]:
            excerpt = str(hit.get("excerpt", ""))
            sid = hit.get("source_record_id")
            signals_section.append(
                f"- retrieval hit (src={sid}): {excerpt[:100]}{'...' if len(excerpt) > 100 else ''}"
            )
        ...
        for sid in (
            [m.get("source_record_id") for m in mention_items]
            + [f.get("source_record_id") for f in file_queue]
            + [e.get("source_record_id") for e in calendar_items]
        ):
            if sid:
                source_map.setdefault(int(sid), set())
        source_lines = []
        for sid in sorted(source_map.keys())[:20]:
            ...
            source_lines.append(
                f"- src={sid} links={', '.join(link_types) if link_types else 'none'}"
            )
        ...
        sections = [
            ...
            if priority_actions
            else "_No high-confidence actions extracted today._",
            "",
            "## Waiting On",
            ...
            "\n".join(meeting_prep)
            if meeting_prep
            else "No meeting prep items found for the configured window.",
            "",
            "## File Review Queue",
            "\n".join(file_review) if file_review else "No current file review candidates found.",
            "",
            "## Project / Workstream Signals",
            "\n".join(signals_section)
            if signals_section
            else "_No current retrieval/body-mention signals._",
            "",
            "## Sources",
            "\n".join(source_lines)
            if source_lines
            else "_No source links available in current context._",
        ]
