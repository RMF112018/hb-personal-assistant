...
def normalize_daily_log_inspection(raw, *, project_key, endpoint_id, correlation_id, fetched_at):
    ...
    return _base(
        ...
        routing_reason="inspections_safety_routed_high"
        if safety
        else "inspections_structured_medium",
        safety_route=safety,
    )
...
def normalize_daily_log_safety_violation(
    raw, *, project_key, endpoint_id, correlation_id, fetched_at
):
    builder = (
        EntityBuilder()
        .add_person(raw.get("created_by"), "created_by")
