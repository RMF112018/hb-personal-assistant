...
RENDERED_NARRATIVE_LOCATION = (
    "<vault>/Construction Intelligence/Phase 09 Rendered Daily Briefs/ "
...
def build_rendered_brief_receipt(
    *,
    ...
    status = "not_run" if validation_passed is None else ("passed" if validation_passed else "failed")
    return {
...
def build_daily_brief_rendered_output_receipt_proof(
    *, evidence_dir: str | None = None, write_evidence: bool = True
    ...
    rendered_receipt = build_rendered_brief_receipt(
        packet=packet,
        rendered_path=(
            "<vault>/Construction Intelligence/Phase 09 Rendered Daily Briefs/2026-06-02-daily-brief.md"
        ),
    ...
    checks: dict[str, bool] = {
        ...
        "excluded_from_source_manifest": cls not in ALLOWLISTED_SOURCE_FAMILIES
        and cls not in _APPROVED_MANIFEST_CATEGORIES,
        "excluded_from_source_linked_proof": cls not in ALLOWLISTED_SOURCE_FAMILIES,
