...
# ---------------------------------------------------------------------------
# Raw content policy (Phase 10A addendum Prompt 01). Supports email_calendar
# mode first; keeps external writeback / auto mutations / cloud LLM prohibited.
# ---------------------------------------------------------------------------

RawContentMode = Literal["disabled", "email_calendar", "all_supported", "all_supported_plus_downstream"]
...
class RawContentSettings(BaseModel):
    ...
    prohibited_without_future_approval: ProhibitedWithoutApproval = Field(default_factory=ProhibitedWithoutApproval)
    model_config = {"extra": "forbid"}
    ...
    def _raw_content_invariants(self) -> "RawContentPolicy":
        ...
        if self.raw_content.mode == "email_calendar":
            ...
            if not (ss.email and ss.calendar):
                raise ValueError("email_calendar mode requires email and calendar starting_sources true")
        ...
        if self.raw_content.mode in ("email_calendar", "disabled") and (d.mcp_allow_raw_content or d.obsidian_allow_raw_content):
            raise ValueError("downstream raw allowances must be false for email_calendar in Prompt 01")
        return self
