"""Phase 10 Prompt 02 — V41 schema constants + read-only schema-status proof.

Single source of truth for the 21 Phase 10 tables and their 13 guard columns, plus a read-only
status report mirroring ``phase_09_schema.build_phase_09_schema_status_report``. Fail-closed on a
stale schema (< V41). Verifies every table is present with the full guard set and that the guard
columns sum to 0 across all rows (a no-raw/no-writeback attestation). Read-only over the DB;
advisory only; never a determination; persists nothing except optional evidence output.
"""
