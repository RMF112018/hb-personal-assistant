...
def record_procore_history_for_record(
    *,
    ...
    Opens ONE connection (or borrows the caller's) and threads it through every
    sub-recorder, so a record costs a single connection instead of 4-5.
    """
...
def get_procore_record_history(
    *, record_key: str, db_path: Optional[Path] = None, conn: Optional[sqlite3.Connection] = None
) -> List[Dict[str, Any]]:
